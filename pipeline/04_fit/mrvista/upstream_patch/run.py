...
if conf['isPRFSynthData']:
    ...
    for sub in sorted(subs):
        ...
        for ses in sorted(sess):
            ...
            # UPSTREAM BUG FIX (paper_braincoder):
            # The original /scripts/run.py only defines opts_file in
            # the real-data branch (~line 278). On the isPRFSynthData
            # path it then calls os.execl("/solve.sh", opts_file, ...)
            # below — opts_file is undefined, NameError gets caught by
            # the `except Exception: die("Failed to exec /solve.sh
            # script!")` handler, which mislabels it as a fork/exec
            # failure. Write out the options.json next to the BIDS
            # output so the MATLAB solver actually has its config.
            opts_file = os.path.join(os.path.dirname(outbids_dir),
                                      'options.json')
            try:
                os.makedirs(os.path.dirname(opts_file), exist_ok=True)
                with open(opts_file, 'w') as fl:
                    json.dump(conf['options'], fl, indent=4)
            except Exception as e:
                die("Could not write options.json (%s): %s"
                    % (opts_file, e))
