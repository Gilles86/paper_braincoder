...
OUTPUT_DIR="$BASEDIR/BIDS/derivatives/prfanalyze-braincoder.dn.A100"
LOG_RUNTIME_FILE="logs/braincoder.dn.a100-runtime-${IDENTIFIER}.txt"
PYTHON_OUTPUT_FILE="logs/braincoder.dn.a100-output-${IDENTIFIER}.txt"

# Choose the appropriate config file
if [ "$IDENTIFIER" == "vanes2019" ]; then
    CONFIG_FILE="$PWD/configs/vanes2019_dn_config.yml"
else
    echo "Error: Unsupported IDENTIFIER value: '$IDENTIFIER'. Exiting."
    exit 1
fi
